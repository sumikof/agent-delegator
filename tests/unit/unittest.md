...
現在、以下の3つのテストが環境制約によりスキップされています：
- `test_load_template()` - テンプレート読み込みテスト（agent-interface.yamlが必要）
- `test_validate_workflow()` - ワークフロースキーマ検証テスト（スキーマファイルが必要）
- `test_validate_agent()` - エージェントスキーマ検証テスト（スキーマファイルが必要）

これらのテストは`@unittest.skip()`デコレータを使用して適切にスキップされており、環境制約が解決された際に自動的に有効化されます。
